...
export default function LogoOverlayEditorWidget() {
  ...
  async function load() {
    ...
    try {
      ...
      setLogos(logoAssets);
      setLogoId((cur) => logoAssets.some((logo) => logo.id === cur) ? cur : logoAssets[0]?.id || "");
    } catch (e) {
  ...
  async function applyLogo() {
    ...
    if (!logo) return setMsg("Selecciona un logo de los assets del cliente.");
    setBusy(true); setMsg("");
    try {
    ...
    {open ? <div className="fixed inset-0 z-[70] flex justify-end bg-zinc-950/30 backdrop-blur-sm">
      ...
      <aside className="relative flex h-full w-full max-w-3xl flex-col bg-white shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6">{loading ? <p className="rounded-2xl bg-zinc-50 px-4 py-4 text-sm text-zinc-600">Cargando editor...</p> : <div className="space-y-5">
          ...
          {logos.length > 0 ? <div><p className="mb-3 text-sm font-semibold text-zinc-900">2. Selecciona el logo</p><div className="grid grid-cols-3 gap-3">{logos.map((logo) => <button key={logo.id} type="button" onClick={() => setLogoId(logo.id)} className={`rounded-2xl border bg-white p-3 text-left transition ${logoId === logo.id ? "border-zinc-950" : "border-zinc-200 hover:bg-zinc-50"}`}><div className="flex h-16 items-center justify-center rounded-xl bg-zinc-50 p-2"><img src={logo.fileUrl} alt={logo.name} className="max-h-full max-w-full object-contain" /></div><p className="mt-2 line-clamp-2 text-xs font-semibold text-zinc-800">{logo.name}</p></button>)}</div></div> : null}
