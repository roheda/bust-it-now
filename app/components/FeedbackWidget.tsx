...
export default function FeedbackWidget() {
  ...
  async function handleResolveItem(item: FeedbackItem) {
    ...
    try {
      await updateDoc(doc(db, "platformFeedback", item.id), {
        status: "done",
      ...
      setSuccess("Mejora marcada como resuelta.");
    } catch (resolveError) {
      console.error(resolveError);
      setError("No se pudo marcar como resuelto. Revisa permisos de Firestore.");
    } finally {
      setResolvingItemId("");
    }
  }
                      ...
                      return (
                        ...
                        >
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <span className={`rounded-full px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.12em] ${isDone ? "bg-emerald-100 text-emerald-800" : "bg-zinc-100 text-zinc-700"}`}>
                              {priorityLabel(item.priority)}
                            </span>
                            <span className={`rounded-full px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.12em] ${statusBadgeClass(item.status)}`}>
                              {isDone ? "✓ " : ""}{statusLabel(item.status)}
                            </span>
                          </div>
                          ...
                          <div className="mt-3 flex justify-end">
                            {isDone ? (
                              <span className="inline-flex h-9 items-center justify-center rounded-2xl bg-emerald-600 px-3 text-xs font-semibold text-white">
                                ✓ Mejora hecha
                              </span>
                            ) : (
                              <button
                                ...
                                className="inline-flex h-9 items-center justify-center rounded-2xl border border-emerald-200 bg-white px-3 text-xs font-semibold text-emerald-700 transition hover:bg-emerald-50 disabled:cursor-not-allowed disabled:opacity-60"
                              >
                                {isResolving ? "Marcando..." : "✓ Marcar resuelto"}
                              </button>
                            )}
                          </div>
                        </article>
